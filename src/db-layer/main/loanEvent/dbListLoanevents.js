...
class DbListLoaneventsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListLoanevents";
    this.emptyResult = true;
    this.objectName = "loanEvents";
  ...
  async transposeResult() {
    for (const loanEvent of this.dbData.items) {
  ...
  createQuery() {
    ...
    return LoanEvent.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (LoanEvent.getCqrsJoins) {
      await LoanEvent.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await LoanEvent.countDocuments(
      this.whereClause,
    );
  }
...
const dbListLoanevents = (input) => {
  const dbGetListCommand = new DbListLoaneventsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListLoanevents;
