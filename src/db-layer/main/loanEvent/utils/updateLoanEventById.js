...
const updateLoanEventById = async (id, dataClause) => {
  try {
    if (!id && dataClause.id) {
      id = dataClause.id;
    ...
    const existingDoc = await LoanEvent.findOne({ _id: id, isActive: true });
    ...
    const dbDoc = await LoanEvent.findOneAndUpdate(
      whereClause,
      dataClause,
      options,
    );
...
module.exports = updateLoanEventById;
