...
const getLoanEventStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(LoanEvent.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            LoanEvent.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await LoanEvent.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingLoanEventStatsByQuery",
      err,
    );
  }
};

module.exports = getLoanEventStatsByQuery;
