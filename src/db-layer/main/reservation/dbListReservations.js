...
class DbListReservationsCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListReservations";
    this.emptyResult = true;
    this.objectName = "reservations";
  ...
  async transposeResult() {
    for (const reservation of this.dbData.items) {
  ...
  createQuery() {
    ...
    return Reservation.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (Reservation.getCqrsJoins) {
      await Reservation.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await Reservation.countDocuments(
      this.whereClause,
    );
  }
...
const dbListReservations = (input) => {
  const dbGetListCommand = new DbListReservationsCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListReservations;
