...
const getReservationStatsByQuery = async (query, stats) => {
  ...
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Reservation.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Reservation.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Reservation.countDocuments(queryWithSoftDelete);
