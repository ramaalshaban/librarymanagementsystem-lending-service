...
class DbListLoansCommand extends DBGetListMongooseCommand {
  constructor(input) {
    super(input);
    this.commandName = "dbListLoans";
    this.emptyResult = true;
    this.objectName = "loans";
  ...
  async transposeResult() {
    for (const loan of this.dbData.items) {
  ...
  createQuery() {
    ...
    return Loan.find(this.whereClause);
  }
  ...
  async getCqrsJoins(item) {
    if (Loan.getCqrsJoins) {
      await Loan.getCqrsJoins(item);
    }
  }

  async setPaginationTotalRowCount() {
    this.paginationTotalRowCount = await Loan.countDocuments(this.whereClause);
  }
...
const dbListLoans = (input) => {
  const dbGetListCommand = new DbListLoansCommand(input);
  return dbGetListCommand.execute();
};

module.exports = dbListLoans;
