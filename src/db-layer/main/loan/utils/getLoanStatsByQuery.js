...
const getLoanStatsByQuery = async (query, stats) => {
  const promises = [];
  const statLabels = [];
  try {
    ...
    for (const stat of stats) {
      ...
      if (stat === "count") {
        promises.push(Loan.countDocuments(queryWithSoftDelete));
        ...
        if (statParts[0] === "sum") {
          ...
          promises.push(
            Loan.aggregate(pipeline).then((result) =>
    ...
    if (promises.length == 0) {
      return await Loan.countDocuments(queryWithSoftDelete);
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenRequestingLoanStatsByQuery",
      err,
    );
  }
};

module.exports = getLoanStatsByQuery;
