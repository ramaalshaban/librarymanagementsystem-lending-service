...
class DeleteLoanMcpController extends LendingMcpController {
  constructor(params) {
    super("deleteLoan", "deleteloan", params);
    this.dataName = "loan";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteLoanManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    name: "deleteLoan",
    description:
      "Soft-delete a loan record if needed for compliance or administrative erasure.",
    parameters: DeleteLoanMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteLoanMcpController = new DeleteLoanMcpController(mcpParams);
      try {
        const result = await deleteLoanMcpController.processRequest();
        //return DeleteLoanMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
