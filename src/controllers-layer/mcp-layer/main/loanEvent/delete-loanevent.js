...
class DeleteLoanEventMcpController extends LendingMcpController {
  constructor(params) {
    super("deleteLoanEvent", "deleteloanevent", params);
    this.dataName = "loanEvent";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeleteLoanEventManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    parameters: DeleteLoanEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const deleteLoanEventMcpController = new DeleteLoanEventMcpController(
        mcpParams,
      );
      try {
        const result = await deleteLoanEventMcpController.processRequest();
        //return DeleteLoanEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
