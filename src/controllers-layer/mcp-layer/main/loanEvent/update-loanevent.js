...
class UpdateLoanEventMcpController extends LendingMcpController {
  constructor(params) {
    super("updateLoanEvent", "updateloanevent", params);
    this.dataName = "loanEvent";
    this.crudType = "update";
  }

  createApiManager() {
    return new UpdateLoanEventManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Update lending event record (rare; audit correction or note update).",
    parameters: UpdateLoanEventMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      mcpParams.headers = headers;
      const updateLoanEventMcpController = new UpdateLoanEventMcpController(
        mcpParams,
      );
      try {
        const result = await updateLoanEventMcpController.processRequest();
        //return UpdateLoanEventMcpController.getOutputSchema().parse(result);
        return {
          content: [
            {
              type: "text",
